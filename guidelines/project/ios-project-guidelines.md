...
- **Reactive Use Cases:** If a Use Case returns a data stream, it MUST return an `AsyncStream` and properly map the underlying repository's reactive stream to the domain model.
    - **Reactive Persistence Mandate:** Use Cases SHOULD return `AsyncStream` when the underlying data is expected to change (e.g., new workouts added). This ensures the UI remains "live" and follows the Offline-First mandate naturally.
    - **Composite Use Cases (Consolidation):** When a screen requires multiple metrics derived from the same source (e.g., OneRM and Volume for Trends), consolidate them into a single Use Case that emits a unified result object. This prevents redundant repository subscriptions and eliminates race conditions between related data points.
