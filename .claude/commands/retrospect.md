...
Analyze the completed feature implementation described in $ARGUMENTS to identify architectural improvements, workflow optimizations, and patterns that should be formalized. Update the project guidelines and expert subagents accordingly.

## Workflow

1. **Select Target Feature:** Identify the feature from `specs/` and its corresponding implementation in `apps/`.
...
5. **Sync Expert Subagents:**
   - Update `.claude/agents/android-expert.md` or `.claude/agents/ios-expert.md` to reflect the refined mandates.
